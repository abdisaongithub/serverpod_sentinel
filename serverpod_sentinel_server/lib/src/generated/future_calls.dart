...
import '../future_calls/step_executor_call.dart' as _i4;
...
class FutureCalls extends _i1.FutureCallDispatch<_FutureCallRef> {
  ...
  void initialize(
    ...
    var registeredFutureCalls = <String, _i1.FutureCall>{
      'StepExecutorCallInvokeFutureCall': StepExecutorCallInvokeFutureCall(),
    };
...
class _FutureCallRef {
  ...
  late final stepExecutorCall = _StepExecutorCallFutureCallDispatcher(
    _invokeFutureCall,
  );
}

class _StepExecutorCallFutureCallDispatcher {
  _StepExecutorCallFutureCallDispatcher(this._invokeFutureCall);

  final _InvokeFutureCall _invokeFutureCall;

  Future<void> invoke(_i2.StepExecutorPayload? payload) {
    return _invokeFutureCall(
      'StepExecutorCallInvokeFutureCall',
      payload,
    );
  }
}

class StepExecutorCallInvokeFutureCall
    extends _i1.FutureCall<_i2.StepExecutorPayload> {
  @override
  _i3.Future<void> invoke(
    _i1.Session session,
    _i2.StepExecutorPayload? payload,
  ) async {
    await _i4.StepExecutorCall().invoke(
      session,
      payload,
    );
  }
